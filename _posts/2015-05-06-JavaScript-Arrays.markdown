...
+ `every()`，依次检验数组中的元素是否全部通过某种测试，全部通过返回 true。**一旦遇到不通过的**数组元素，**立即停止检查**剩下的元素，返回 false。
+ `some()`，依次检查数组中的元素是否有通过某种测试的，**一旦遇到通过的**数组元素，**立即停止检查**剩下的元素，返回 true。全都不通过，返回 false。
