...
因为**栅格布局优先**，所以_float_ 、 _clear_ 和 _vertical-align_ 属性对于栅格 item 不起作用。
...
栅格轨道是栅格上任意两条线之间的space。通过 _grid-template-columns_ 和 _grid-template-rows_ 定义**列**和**行**的宽度。
...
3. 定义大型栅格多条轨道时，可使用 `repeat(重复次数, 重复pattern)` 实现重复全部或部分track列表。如：

    grid-template-columns: repeat(3, 33.33%); // 3列，每列33.33%宽
    grid-template-rows: repeat(2, 50px); // 2行，每行50px高
...
栅格单元格之间的空隙/间距，通过 _grid-column-gap_ 和 _grid-row-gap_ 属性创建（缩写属性为 _grid-gap_）。
