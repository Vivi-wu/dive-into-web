...
1. `@for $var from <start> through <end>`， &lt;start&gt; 和 &lt;end&gt; 是返回 integer 的 SassScript 表达式。通常是 increment，如果 start 值比 end 大，则采用 decrement。
...
是 SassScript 的变量名，separated by commas。
