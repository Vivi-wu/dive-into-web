...
24. 在 Canvas 元素上绘制文本，字符以像素方式固定在上面，文本成为图像的一部分，除非重新绘制 Canvas 绘图区域，否则无法改变文本内容。

注意：如果你的图像显示**需要显著的交互行为**，可以考虑使用 SVG 代替。

## SVG （Scalable Vector Graphic 可伸缩矢量图形）

在栅格图形/位图中，图像由一组二维像素网格表示。PNG 和 JPEG 是常见的栅格图形格式。Canvas 2d API 就是一款栅格图形API。

在矢量图形中，图像由数学描述的几何形状表示。SVG 是一种基于XML用于描述二维矢量图形的图形格式。HTML5 引入了内联 SVG，因此 SVG 元素可以直接出现在 HTML 标记中。

特点：

1. SVG 绘制的文本（text 元素中的文本）是鼠标可选的，也可以被搜索引擎获取（实现基于内容的图像搜索），而 Canvas 做不到。

2. SVG 是用来创建视觉结构的语言，同 HTML 一样，它支持脚本操作和添加样式。易于编辑。

3. 放大、旋转或用其他手段变换 SVG 内容的时候，渲染程序会立即重绘所有构成图像的线条，因此缩放 SVG 不会导致其质量下降。因为 SVG 文档在呈现时会**保留构成它的矢量信息**，而像素图形在图像完成绘制后便丢失了构成图像的路径和图形（基本信息）。

4. SVG 绘制形状对象时，是按照对象在文档中出现的顺序进行的。

5. 支持多种滤镜`<filter>`和特殊效果，在不改变图像内容的情况下，可以实现模糊、阴影、发光等。
...
1. `<g>` 代表“组”。可用于将多个元素结合起来，使它们作为一个整体进行变换或链接。

2. `<defs>` 定义留待将来使用的内容，定义的组本身不可见。`<use>` 元素用来链接到 `<defs>` 元素定义的内容。借助这两个元素，可以实现内容的复用。

### 绘制基本几何图形

| 图形 | 用法 | 其他 |
| --- | --- | --- |
| 矩形 | `<rect width="100" height="120" />` | 设置 rx、ry 属性可以创建圆角矩形。 |
| 圆形 | `<circle cx="50" cy="50" r="30" />` |
| 椭圆 | `<ellipse cx="50" cy="50" rx="30" ry="10" />` |
| 线段 | `<line x1="10" y1="10" x2="50" y2="50" />` |
| 折线 | `<polyline points="10 10 50 50 90 10" />` |
| 多边形 | `<polygon points="10 10 50 50 90 10" />` |
| 路径 | `<path d="M 10 10 L 50 50 L 90 10 Z" >` | _d_ 代表数据 data，数据中的 _M_ 移至 moveto 坐标位置，_L_ 绘线至 lineto，_Q_ 代表二次曲线，_Z_ 代表闭合路径，_A_ 代表绘制椭圆弧。|
| 文本 | `<text x="50" y="50">Hello World</text>` |
