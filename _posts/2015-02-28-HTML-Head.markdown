...
当rel的值为 preload，配合属性 _as_（资源类型），指定你的页面在 loading 后马上（你想要在页面加载生命周期的早期、浏览器主渲染流程开始时预加载）需要的资源。
...
The `<base>` tag 该标签指定了**一个HTML文档中所有相对URL的基地址和target目标地址**(具体取值参考links章节)。比如 `<img>` 标签引用images文件夹里的图片，其属性 _src_的值可以直接写文件名。
