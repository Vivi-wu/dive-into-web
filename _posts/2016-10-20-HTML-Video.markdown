...
2. 主流视频容器支持视频格式：Audio Video Interleave(.avi)、Flash Video(.flv)、MPEG 4(.mp4)、Matroska(.mkv)、Ogg(.ogv)、webm
...
7. 为更好地支持各种浏览器，通过 `<source>` 元素引入多种视频格式。添加 _type_ 特性，如 type＝“audio/mpeg”。如果 type 特性中指定的类型与源文件不匹配，浏览器可能会拒绝播放。因为一个媒体容器可能支持多种类型的编解码器，声明的源文件扩展名可能会让浏览器误以为自己支持或不支持某种类型。
...
+ _loop_，值与属性同名，可不赋值。循环播放——文件播放到尾部时自动 seek back 到开始部分。
+ _muted_，值与属性同名，可不赋值。设置视频的音频输出为静音。
+ _poster_，值为 url。封面——视频下载时或用户点击播放按钮前显示的图像。如果没有设置该特性，那么直到第一帧可用前，视频元素区域显示 nothing（空白），之后第一帧会作为 poster frame 显示。
