...
为了解决这些问题，Hook 使你在**函数组件**里，也可以使用更多的 React 特性。
...
state hook 唯一的入参是**初始**state。

一个组件内可以多次使用 state hook 来声明多个 state 变量。

state 更新总是 replace 而不是 merge。
...
使用多个 effect 来分离关注点/不相关的逻辑。

React 将按照 effect 声明的顺序**依次调用**组件中的每一个 effect。 

#### 无需 cleanup 的 effect

一些在 DOM 更新后运行的代码，如：网络请求、手动修改 DOM 等，在执行后就可以被忘掉，我们称之为不需要清理的 effect。
...
每个 effect 通过**返回一个函数**来指定 React 在组件销毁时如何清除副作用。
...
如果希望只运行和清理一个 effect 一次，可以传入空数组`([])`. 告诉 React 这个 effect 不依赖任何 props 或 state 的值.

注意：这样做 effect 内部的 props 或 state 的值将**永远是初始值**。

### 自定义 Hook

Hook 是一种复用状态逻辑的方式，每次调用都有一个完全独立的 state，因此可以在单个组件中多次调用同一个自定义 Hook。

好处：不会在 tree 上添加更多的组件。
...
使用底层 `<Router>` 最常见的情况是将自定义历史记录与状态管理库（如 Redux 或 Mobx）进行同步。但是 React Router 并不是一定要跟状态管理库一起使用，它仅用于深度集成 history。
